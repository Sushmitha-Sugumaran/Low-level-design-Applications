...
public class MoviesSection {
    ...
    public static void displayMovies(String schedule) {
        System.out.println("All movies list given below ...");
        boolean foundMovies = false;
        for (int i = 0; i < moviesList.size(); i++) {
            if (moviesList.get(i).getShowSchedule().equals(schedule)) {
                System.out.println(moviesList.get(i).getMoviesId() + "-->" + moviesList.get(i).getLanguage() + " , " + moviesList.get(i).getMovieName() + " , " +
                        moviesList.get(i).getGenre() + " , " + moviesList.get(i).getSeatCount() + " , " + "Per Ticket Cost : " + moviesList.get(i).getPerTicketPrice());
                foundMovies = true;
            }
        }
        if (!foundMovies) {
