...
public class EventsSection {
    ...
    public static void displayEvents(String schedule) {
        System.out.println("All event list given below ...");
        boolean foundMovies = false;
        for (int i = 0; i < eventsList.size(); i++) {
            if (eventsList.get(i).getShowSchedule().equals(schedule)) {
                System.out.println(eventsList.get(i).getEventsId() + "-->" + eventsList.get(i).getLanguage() + " , " + eventsList.get(i).getEventName() + " , " +
                        eventsList.get(i).getGenre() + " , " + eventsList.get(i).getSeatCount() + " , " + "Per Ticket Cost : " + eventsList.get(i).getPerTicketPrice());
                foundMovies = true;
            }
        }
        if (!foundMovies) {
